...
const ROCK_PATH = "/Sunnyside_World_ASSET_PACK_V2.1/Sunnyside_World_Assets/Elements/Crops/rock.png";
const WHEAT_PATH = "/Sunnyside_World_ASSET_PACK_V2.1/Sunnyside_World_Assets/Elements/Crops/wheat_02.png";
  ...
  public render(canvas: HTMLCanvasElement, engine: SurvivalEngine) {
    ...
    if (this.tilesetImg && this.tilesetImg.complete) {
      for (let ty = minTileY; ty <= maxTileY; ty++) {
        for (let tx = minTileX; tx <= maxTileX; tx++) {
          ...
          ctx.drawImage(this.tilesetImg, grassCrop.x, grassCrop.y, grassCrop.w, grassCrop.h, screen.sx, screen.sy, tileW, tileH);

          // 2. Draw specialized terrain overlay on top if not base grass
          if (tile.terrain !== "grass_textured_01") {
            const crop = TILE_CROPS[tile.terrain] || TILE_CROPS["grass_textured_01"];

            if (tile.rotation) {
              ctx.save();
              ctx.translate(screen.sx + tileW / 2, screen.sy + tileH / 2);
              ctx.rotate(tile.rotation);
              ctx.drawImage(this.tilesetImg, crop.x, crop.y, crop.w, crop.h, -tileW / 2, -tileH / 2, tileW, tileH);
              ctx.restore();
            } else {
              ctx.drawImage(this.tilesetImg, crop.x, crop.y, crop.w, crop.h, screen.sx, screen.sy, tileW, tileH);
            }
          }
        }
      }
      ...
      for (const farm of village.data.farms) {
        for (const cell of farm.cells) {
          ...
          if (this.soilImg && this.soilImg.complete && this.soilImg.naturalWidth > 0) {
            ...
            ctx.drawImage(this.tilesetImg, dirtCrop.x, dirtCrop.y, dirtCrop.w, dirtCrop.h, screen.sx, screen.sy, tileW, tileH);
          }
        }
      }
    }
    ...
    for (const village of engine.worldManager.villages) {
      ...
      for (const deco of village.data.decorations) {
        ...
        const screen = worldToScreen(wx, wy);
        if (deco.id in TILE_CROPS && this.tilesetImg && this.tilesetImg.complete) {
          const crop = TILE_CROPS[deco.id];
          ctx.drawImage(this.tilesetImg, crop.x, crop.y, crop.w, crop.h, screen.sx, screen.sy, cellSize, cellSize);
        } else if (deco.id in SPRITE_DEFS) {
          const def = SPRITE_DEFS[deco.id];
          const img = this.getSpriteImage(deco.id);
          if (img) {
            const isSmall = deco.id === "acorn_deco_01" || deco.id === "truffle_deco_01";
            ...
            ctx.drawImage(img, screen.sx + (cellSize - dw) / 2, screen.sy + (cellSize - dh) / 2, dw, dh);
          }
        }
      }
    }
    ...
    for (const struct of engine.placedStructures) {
      if (struct.type === "wood_floor") {
        ...
        ctx.lineWidth = 1;
        ctx.strokeRect(screen.sx + 1, screen.sy + 1, cellSize - 2, cellSize - 2);
      }
    }
    ...
    for (const item of engine.droppedItems) {
      ...
      ctx.beginPath();
      ctx.ellipse(screen.sx + cellSize / 2, screen.sy + cellSize / 2 + 6, 6 * (this.zoom / 2), 3 * (this.zoom / 2), 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillText(def?.icon || "📦", screen.sx + cellSize / 2, screen.sy + cellSize / 2 + bobY);
    }
    ...
    for (const village of engine.worldManager.villages) {
      for (const house of village.data.houses) {
        ...
        if (wx + house.footprintW < minTileX - 3 || wx > maxTileX + 3 || wy + house.footprintH < minTileY - 3 || wy > maxTileY + 3) {
          continue;
        }
        ...
        depthList.push({
          ...
          draw: () => {
            const screen = worldToScreen(wx, wy);
            const crop = HOUSE_CROPS[house.id] || HOUSE_CROPS["house_cottage_01"];

            if (crop && this.housesImg && this.housesImg.complete && this.housesImg.naturalWidth > 0) {
              ...
              ctx.beginPath();
              ctx.ellipse(dx + dw / 2, screen.sy + footprintPixelH - 2, dw * 0.46, 8, 0, 0, Math.PI * 2);
              ctx.fill();
      ...
      if (village.data.farmObjects) {
        for (const obj of village.data.farmObjects) {
          ...
          depthList.push({
            ...
            draw: () => {
              const screen = worldToScreen(wx, wy);
              if (obj.id in TILE_CROPS && this.tilesetImg && this.tilesetImg.complete) {
                ...
                if (img) {
                  ...
                  const dw = def.w * scale;
                  const dh = def.h * scale;
                  ctx.drawImage(img, screen.sx + (cellSize - dw) / 2, screen.sy + cellSize - dh, dw, dh);
                }
              }
            },
          });
      ...
      for (const well of village.data.wells) {
        ...
        depthList.push({
          ...
          draw: () => {
            const screen = worldToScreen(wx, wy);
            const img = this.getSpriteImage(well.id) || this.getSpriteImage("farm_well_covered");
            if (img) {
              const def = SPRITE_DEFS[well.id] || SPRITE_DEFS["farm_well_covered"];
          ...
          depthList.push({
            ...
            draw: () => {
              ...
              if (this.wheatImg && this.wheatImg.complete) {
                ...
                ctx.drawImage(this.wheatImg, screen.sx + (cellSize - cropW) / 2, screen.sy + cellSize - cropH, cropW, cropH);
              }
            },
          });
        }
    ...
    for (let dcy = -2; dcy <= 2; dcy++) {
      for (let dcx = -2; dcx <= 2; dcx++) {
        const chunk = engine.worldManager.getOrCreateChunk(centerChunkX + dcx, centerChunkY + dcy);
        for (const res of chunk.resources) {
          if (res.isDepleted) continue;
          if (res.x < minTileX - 3 || res.x > maxTileX + 3 || res.y < minTileY - 3 || res.y > maxTileY + 3) continue;

          depthList.push({
            yOrder: res.y + res.h,
            draw: () => {
              const screen = worldToScreen(res.x, res.y);
              const shakeOffset = res.shakeTimer && res.shakeTimer > 0 ? (Math.random() * 4 - 2) : 0;

              if (res.type === "tree") {
                // Ground shadow
                ctx.fillStyle = "rgba(0,0,0,0.35)";
                ctx.beginPath();
                ctx.ellipse(screen.sx + cellSize, screen.sy + cellSize * 1.6, cellSize * 0.9, cellSize * 0.4, 0, 0, Math.PI * 2);
                ctx.fill();
                ...
                if (this.treeBushImg && this.treeBushImg.complete) {
                  ...
                  ctx.drawImage(this.treeBushImg, crop.x, crop.y, crop.w, crop.h, dx + shakeOffset, dy, treeW, treeH);
                }
              } else if (res.type === "rock" || res.type === "iron_rock") {
                // Ground shadow
                ctx.fillStyle = "rgba(0,0,0,0.32)";
                ctx.beginPath();
                ctx.ellipse(screen.sx + cellSize / 2, screen.sy + cellSize * 0.75, cellSize * 0.45, cellSize * 0.22, 0, 0, Math.PI * 2);
                ctx.fill();
                ...
                if (this.rockImg && this.rockImg.complete) {
                  ...
                  ctx.drawImage(this.rockImg, dx + shakeOffset, dy, rockW, rockH);

                  if (res.type === "iron_rock") {
                    ctx.fillStyle = "rgba(249, 115, 22, 0.8)";
                    ctx.fillRect(screen.sx + cellSize * 0.4, screen.sy + cellSize * 0.35, 3 * (this.zoom / 2), 3 * (this.zoom / 2));
                    ctx.fillStyle = "#fed7aa";
                    ctx.fillRect(screen.sx + cellSize * 0.6, screen.sy + cellSize * 0.5, 2 * (this.zoom / 2), 2 * (this.zoom / 2));
                  }
                }
                ...
                if (this.treeBushImg && this.treeBushImg.complete) {
                  ...
                  ctx.drawImage(this.treeBushImg, crop.x, crop.y, crop.w, crop.h, screen.sx, screen.sy + cellSize - bushH, bushW, bushH);
                }
              } else if (res.type === "crop") {
                if (this.wheatImg && this.wheatImg.complete) {
                  const cropW = cellSize;
                  const cropH = cellSize * 1.2;
                  ctx.drawImage(this.wheatImg, screen.sx, screen.sy - cropH * 0.2, cropW, cropH);
                }
              }
              ...
              if (res.health < res.maxHealth) {
                ...
                ctx.fillRect(barX, barY, barW * (res.health / res.maxHealth), barH);
              }
            },
          });
        }
      ...
      depthList.push({
        ...
        draw: () => {
          ...
          if (struct.type === "wood_wall") {
            ...
            ctx.fillRect(screen.sx, screen.sy - cellSize * 0.4, cellSize, cellSize * 1.4);
            ctx.fillStyle = "#92400e";
            ctx.fillRect(screen.sx + 2, screen.sy - cellSize * 0.3, cellSize - 4, cellSize * 1.2);
            ...
            ctx.fillText(struct.isOpen ? "OPEN" : "DOOR", screen.sx + 2, screen.sy + cellSize / 2);
          } else if (struct.type === "campfire") {
            ctx.fillStyle = "rgba(0,0,0,0.4)";
            ctx.beginPath();
            ctx.ellipse(screen.sx + cellSize / 2, screen.sy + cellSize * 0.8, cellSize * 0.4, cellSize * 0.2, 0, 0, Math.PI * 2);
            ctx.fill();

            const flicker = Math.sin(Date.now() * 0.01) * 3;
            ctx.fillStyle = "#f97316";
            ctx.beginPath();
            ctx.arc(screen.sx + cellSize / 2, screen.sy + cellSize / 2 - 2, (cellSize * 0.35) + flicker, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#fef08a";
            ctx.beginPath();
            ctx.arc(screen.sx + cellSize / 2, screen.sy + cellSize / 2 - 4, cellSize * 0.18, 0, Math.PI * 2);
            ctx.fill();
          } else if (struct.type === "workbench") {
            ctx.fillStyle = "#9a3412";
            ctx.fillRect(screen.sx + 2, screen.sy + 2, cellSize - 4, cellSize - 4);
            ctx.fillStyle = "#fbbf24";
            ctx.font = `${14 * (this.zoom / 2)}px sans-serif`;
            ctx.textAlign = "center";
            ctx.fillText("🛠️", screen.sx + cellSize / 2, screen.sy + cellSize / 2);
          } else if (struct.type === "chest") {
            ctx.fillStyle = "#854d0e";
            ctx.fillRect(screen.sx + 2, screen.sy + 4, cellSize - 4, cellSize - 8);
            ctx.fillStyle = "#fef08a";
            ctx.fillRect(screen.sx + cellSize / 2 - 2, screen.sy + cellSize / 2 - 2, 4, 4);
          }
        },
      });
    }
    ...
    if (engine.worldTime.lightLevel < 0.95) {
      ...
      if (lctx) {
        ...
        lctx.beginPath();
        lctx.arc(playerScreen.sx + cellSize / 2, playerScreen.sy + cellSize / 2, playerRadius, 0, Math.PI * 2);
        lctx.fill();
        ...
        for (const struct of engine.placedStructures) {
          if (struct.type === "campfire") {
            ...
            lctx.beginPath();
            lctx.arc(screen.sx + cellSize / 2, screen.sy + cellSize / 2, campRadius, 0, Math.PI * 2);
            lctx.fill();
          }
        }
    ...
    if (engine.isBuildMode && engine.buildPiece) {
      ...
      ctx.save();
      ctx.fillStyle = engine.isBuildPreviewValid ? "rgba(34, 197, 94, 0.4)" : "rgba(239, 68, 68, 0.4)";
      ctx.fillRect(screen.sx, screen.sy, cellSize, cellSize);
